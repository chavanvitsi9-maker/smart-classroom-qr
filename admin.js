  ...
  onValue(subjectsRef, async (snapshot) => {
    if (!snapshot.exists()) {
      ...
      const initialSubjects = {
        CS101: { code: "CS101", name: "วิทยาการคอมพิวเตอร์เบื้องต้น", defaultStartTime: "09:00", room: "Lab 401", createdAt: Date.now() },
        INT201: { code: "INT201", name: "การพัฒนาเว็บแอปพลิเคชัน", defaultStartTime: "10:30", room: "Lab 1", createdAt: Date.now() },
        ENG102: { code: "ENG102", name: "ภาษาอังกฤษเพื่อการสื่อสาร", defaultStartTime: "13:00", room: "ห้อง 205", createdAt: Date.now() },
        MATH104: { code: "MATH104", name: "สถิติและความน่าจะเป็น", defaultStartTime: "15:00", room: "ห้อง 302", createdAt: Date.now() }
      };
...
function updateSubjectDropdowns() {
  ...
  qrSubjectSelect.innerHTML = allSubjects.map(s => {
    return `<option value="${s.code}" data-name="${s.name}" data-time="${s.defaultStartTime || '09:00'}">${s.code} - ${s.name}</option>`;
  }).join("");
  ...
  if (filterSubject) {
    let filterOpts = `<option value="ALL">ทุกวิชา (All Subjects)</option>`;
  ...
  createSubjectForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (subjectCreationAlert) {
        ...
        subjectCreationAlert.innerHTML = `✅ เพิ่มวิชา <strong>${code} - ${name}</strong> สำเร็จแล้ว!`;
